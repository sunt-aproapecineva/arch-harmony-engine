...
const CellPopover: React.FC<{ userName: string; moduleName: string; lessons: { title: string; completed: boolean; completedAt?: string }[]; onClose: () => void; }> = ({ userName, moduleName, lessons, onClose }) => (
  <motion.div style={{ position: 'fixed', inset: 0, zIndex: 50 }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}>
    ...
    <motion.div initial={{ scale: 0.95, y: 16 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 16 }}
      style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: 24, width: '90%', maxWidth: 420, zIndex: 51 }}
      onClick={e => e.stopPropagation()}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 16 }}>
        <div>
          <div style={{ fontSize: 11, color: 'var(--fg-3)', marginBottom: 4 }}>{userName}</div>
          <h3 className="font-aboreto" style={{ fontSize: 16, color: 'var(--fg)' }}>{moduleName}</h3>
        </div>
...
export const AdminProgress: React.FC = () => {
  ...
  const exportCSV = () => {
    const headers = ['Email', 'Nume', 'Rol', 'Tarif', 'Data înregistrării', ...modules.map(m => m.title), 'Total lecții', '% progres'];
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 24px' }}>
            ...
            {[...users].sort((a, b) => getUserOverallPct(b.id) - getUserOverallPct(a.id)).slice(0, 5).map((user, idx) => {
              ...
              return (
                <div key={user.id} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <span className="font-aboreto" style={{ width: 20, textAlign: 'center', fontSize: 12, color: idx === 0 ? 'var(--gold)' : 'var(--fg-3)' }}>{idx + 1}</span>
                  <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'linear-gradient(135deg, #1A5C38, #0f3d22)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: '#C4F0E4' }}>
                    ...
                    {modules.map(mod => {
                      ...
                      return (
                        <td key={mod.id} style={{ padding: '10px', textAlign: 'center' }}
                          ...
                          <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minWidth: 44, padding: '3px 8px', borderRadius: 8, fontSize: 11, fontWeight: 600, background: cs.background, color: cs.color, border: `1px solid ${pct === 0 ? 'rgba(255,255,255,0.06)' : pct === 100 ? 'rgba(74,222,128,0.25)' : 'rgba(201,169,110,0.25)'}`, cursor: 'pointer' }}>
                            {pct === 0 ? '—' : `${pct}%`}
                          </div>
                        </td>
                      );
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {modules.map((mod, i) => (
              <div key={mod.id} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <span className="font-aboreto" style={{ width: 20, fontSize: 11, color: 'var(--fg-3)', textAlign: 'right' }}>{mod.order_index}</span>
      ...
      <AnimatePresence>
        {modal && modalData && (
          <CellPopover userName={modalData.userName} moduleName={modalData.moduleName} lessons={modalData.lessons} onClose={() => setModal(null)} />
        )}
      </AnimatePresence>
    </div>
  );
