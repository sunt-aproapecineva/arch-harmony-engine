// AUTO-GENERATED content snapshot of published modules/lessons.
// Used as an offline-safe base layer so published lessons never disappear
// when the network or the database is momentarily unavailable.
// Regenerate by exporting the modules/lessons tables.
export const CONTENT_SNAPSHOT = {
  "mods": [
    {
      "id": "1f2cd77d-826b-42be-89e8-3ce7dc2d30d5",
      "order_index": 0,
      "title": "Proiectul Casei: Diagnostică & Plan",
      "subtitle": "Înainte să sapi fundația, desenezi proiectul.",
      "description": "Diagnostic complet al afacerii tale + plan personalizat pe 8 săptămâni.",
      "etapa": "Etapa 0",
      "saptamana": "Săptămâna 0"
    },
    {
      "id": "89415b87-8739-47e2-8439-0fc8296acd35",
      "order_index": 1,
      "title": "Fundația: Claritate & Poziționare",
      "subtitle": "Dacă fundația e strâmbă, toată casa e strâmbă.",
      "description": "Rol, misiune, viziune, valori, identitate.",
      "etapa": "Etapa 1",
      "saptamana": "Săptămâna 1"
    },
    {
      "id": "32656a4c-d805-4dbf-8e27-6c95ced6766d",
      "order_index": 2,
      "title": "Pereții Portanți: Structură Organizațională",
      "subtitle": "Pereții portanți împart casa în camere și o țin în picioare la cutremur.",
      "description": "Organigramă, funcții, fișe de rol.",
      "etapa": "Etapa 2",
      "saptamana": "Săptămâna 2"
    },
    {
      "id": "98bb20f0-9a21-4cdd-ac7b-44ee57a4ae8c",
      "order_index": 3,
      "title": "Instalațiile: Procese Operaționale",
      "subtitle": "Fără țevi, fără curent — nu poți locui în casă.",
      "description": "Procese, standardizare, instrucțiuni.",
      "etapa": "Etapa 3",
      "saptamana": "Săptămânile 3–4"
    },
    {
      "id": "fceaf485-9834-4e78-8b77-f37a37f798f9",
      "order_index": 4,
      "title": "Contoarele: Control & KPI",
      "subtitle": "Fără contoare nu conduci — ghicești.",
      "description": "Produs finit, indicatori, tablou de bord.",
      "etapa": "Etapa 4",
      "saptamana": "Săptămâna 5"
    },
    {
      "id": "66e80264-74e8-499d-bc86-827ff4547e84",
      "order_index": 5,
      "title": "Predarea Cheilor: Delegare Reală",
      "subtitle": "Constructorul nu locuiește în casa pe care a construit-o.",
      "description": "Delegare de rezultate, retragere din operațional.",
      "etapa": "Etapa 5",
      "saptamana": "Săptămânile 6–7"
    },
    {
      "id": "f728d8b1-c836-4a14-aa29-27c2da892398",
      "order_index": 6,
      "title": "Recepția Finală: Management & Scalare",
      "subtitle": "La recepție verifici că totul funcționează și semnezi.",
      "description": "Nivelul de management, owner de sistem.",
      "etapa": "Etapa 6",
      "saptamana": "Săptămâna 8"
    },
    {
      "id": "e111821a-e2ec-44ac-b9d0-208990a6f1c6",
      "order_index": 0,
      "title": "Ce Construiești de Fapt",
      "subtitle": "Claritate înainte de orice construcție.",
      "description": "Înainte să vorbim despre nișă, validare sau vânzări, trebuie să existe claritate despre ce construim și de ce ordinea pașilor contează mai mult decât energia cu care pornești.",
      "etapa": "Etapa 0",
      "saptamana": "Săptămâna 0"
    }
  ],
  "lessons": [
    {
      "id": "61371916-c69d-4f73-a045-15f4b4404524",
      "module_id": "1f2cd77d-826b-42be-89e8-3ce7dc2d30d5",
      "order_index": 0,
      "title": "Bun venit. Hai să începem.",
      "description": "Mesaj de bun venit în practicum: ce construiești în 8 săptămâni, cum să folosești platforma și cum să lucrezi direct pe firma ta.",
      "video_url": null,
      "pdf_url": null,
      "duration_min": 0,
      "is_published": true
    },
    {
      "id": "363505c5-70c4-477e-92df-527ef526e9a3",
      "module_id": "1f2cd77d-826b-42be-89e8-3ce7dc2d30d5",
      "order_index": 1,
      "title": "Unde ești și de ce ești blocat acolo",
      "description": "Majoritatea antreprenorilor joacă toate 3 roluri simultan - Asta e rădăcina tuturor problemelor",
      "video_url": "https://youtu.be/nWW0xHxytgs",
      "pdf_url": null,
      "duration_min": 40,
      "is_published": true
    },
    {
      "id": "cdd38c07-44ef-4dfd-a0f5-c33e9724df2d",
      "module_id": "1f2cd77d-826b-42be-89e8-3ce7dc2d30d5",
      "order_index": 2,
      "title": "Parteneriate",
      "description": null,
      "video_url": "https://youtu.be/J-J-U8pGoqE",
      "pdf_url": null,
      "duration_min": 38,
      "is_published": true
    },
    {
      "id": "7253dacc-5349-43d4-ac62-8cbcace39965",
      "module_id": "32656a4c-d805-4dbf-8e27-6c95ced6766d",
      "order_index": 1,
      "title": "Produsul final al fiecărei funcții.",
      "description": "De ce desenezi VIITORUL, nu prezentul. Organigrama actuală vs organigrama la 3 ani.",
      "video_url": "https://youtu.be/9ZOKyML5_pk",
      "pdf_url": null,
      "duration_min": 60,
      "is_published": true
    },
    {
      "id": "7897b537-4b9c-4548-99bc-8765edd3b064",
      "module_id": "32656a4c-d805-4dbf-8e27-6c95ced6766d",
      "order_index": 2,
      "title": "Organigrama",
      "description": "De la 5 la 500 de angajați, aceleași 7 funcții există. Cum le identifici și cum le acoperi în stadiul tău actual.",
      "video_url": "https://youtu.be/qSiLBz9nwIU",
      "pdf_url": null,
      "duration_min": 46,
      "is_published": true
    },
    {
      "id": "ca6166d2-b11c-4ece-af22-cc568572adcd",
      "module_id": "32656a4c-d805-4dbf-8e27-6c95ced6766d",
      "order_index": 3,
      "title": "Matricea decizională",
      "description": "Continuarea lecției despre cele 7 funcții obligatorii: aplicare practică și exemple.",
      "video_url": "https://youtu.be/UYzWC4ciDeM",
      "pdf_url": null,
      "duration_min": 25,
      "is_published": true
    },
    {
      "id": "9108fe94-5806-43f8-b4f8-1f0d324cc48e",
      "module_id": "66e80264-74e8-499d-bc86-827ff4547e84",
      "order_index": 0,
      "title": "Ce e delegarea reală: responsabilitate și autoritate",
      "description": "Delegarea reală are 3 ingrediente: responsabilitate pentru un rezultat, autoritate de decizie și un proces scris.",
      "video_url": "https://youtu.be/lNmLbQYr-cE",
      "pdf_url": null,
      "duration_min": 21,
      "is_published": true
    },
    {
      "id": "01c921ba-1a66-460a-9189-dcfd5b432dc8",
      "module_id": "66e80264-74e8-499d-bc86-827ff4547e84",
      "order_index": 1,
      "title": "Cum ieși treptat din operațional, fără haos?",
      "description": "Formula retragerii: alegi procesul, transferi complet, confirmi în rodaj și faci pasul înapoi când cifra e bună constant, fără tine.",
      "video_url": "https://www.youtube.com/watch?v=3tKMRzJsbAY",
      "pdf_url": null,
      "duration_min": 19,
      "is_published": true
    },
    {
      "id": "f1cc1baa-1f8d-4560-91ca-71f65cbf3768",
      "module_id": "66e80264-74e8-499d-bc86-827ff4547e84",
      "order_index": 2,
      "title": "Greșelile controlate și corectarea sistemului",
      "description": "De la „cine e vinovat\" la „ce reparăm în sistem\" — cu cei 5 de ce și registrul de greșeli.",
      "video_url": "https://www.youtube.com/watch?v=OWdOX0LMoZU",
      "pdf_url": null,
      "duration_min": 28,
      "is_published": true
    },
    {
      "id": "793aed3f-6d99-4136-949c-b7747d9da764",
      "module_id": "89415b87-8739-47e2-8439-0fc8296acd35",
      "order_index": 1,
      "title": "Misiunea Viziunea Valorile",
      "description": null,
      "video_url": "https://youtu.be/xITojkYD7xw",
      "pdf_url": null,
      "duration_min": 24,
      "is_published": true
    },
    {
      "id": "65247afa-5732-40d8-8cbe-044eb3d8513c",
      "module_id": "89415b87-8739-47e2-8439-0fc8296acd35",
      "order_index": 2,
      "title": "Cum comunici echipei, Fundația companiei?",
      "description": null,
      "video_url": "https://youtu.be/BBd1kcMvxmc",
      "pdf_url": null,
      "duration_min": 20,
      "is_published": true
    },
    {
      "id": "4fd44a73-352a-405b-aabe-6a27e50efb87",
      "module_id": "98bb20f0-9a21-4cdd-ac7b-44ee57a4ae8c",
      "order_index": 0,
      "title": "Cum angajăm corect",
      "description": null,
      "video_url": "https://youtu.be/K9h1QLDjC9c",
      "pdf_url": null,
      "duration_min": 28,
      "is_published": true
    },
    {
      "id": "9f694a15-3a47-4054-9f61-bcc7be18c5ad",
      "module_id": "98bb20f0-9a21-4cdd-ac7b-44ee57a4ae8c",
      "order_index": 1,
      "title": "Ce este un proces și cum îl documentezi?",
      "description": null,
      "video_url": "https://youtu.be/6oJwKh3ydM4",
      "pdf_url": null,
      "duration_min": 32,
      "is_published": true
    },
    {
      "id": "b4171c38-bb24-4552-b10f-cacfdb8099b9",
      "module_id": "98bb20f0-9a21-4cdd-ac7b-44ee57a4ae8c",
      "order_index": 2,
      "title": "Harta proceselor firmei tale",
      "description": null,
      "video_url": "https://www.youtube.com/watch?v=8wgFZ0BuWjw",
      "pdf_url": null,
      "duration_min": 14,
      "is_published": true
    },
    {
      "id": "c6cff6cd-fec9-4fcd-b44d-d98f8bdd0087",
      "module_id": "98bb20f0-9a21-4cdd-ac7b-44ee57a4ae8c",
      "order_index": 3,
      "title": "Ce este un flux și tipurile lui",
      "description": null,
      "video_url": "https://www.youtube.com/watch?v=j7VRnpGQn7s",

      "pdf_url": null,
      "duration_min": 12,
      "is_published": true
    },
    {
      "id": "97b40883-55a4-4574-b55e-e3996ff146a8",
      "module_id": "f728d8b1-c836-4a14-aa29-27c2da892398",
      "order_index": 0,
      "title": "De la a construi sistemul la a conduce sistemul",
      "description": null,
      "video_url": "https://youtu.be/_tofm3bzJ-M",
      "pdf_url": null,
      "duration_min": 14,
      "is_published": true
    },
    {
      "id": "96f39b68-f3ab-48dc-a072-69acd7ef463d",
      "module_id": "f728d8b1-c836-4a14-aa29-27c2da892398",
      "order_index": 1,
      "title": "Ritmul de conducere: ședința, ciclul, planul",
      "description": null,
      "video_url": "https://youtu.be/UmkHNaNbi58",
      "pdf_url": null,
      "duration_min": 18,
      "is_published": true
    },
    {
      "id": "fda7f17c-d66e-4eed-a22b-85f78ea0d7ce",
      "module_id": "f728d8b1-c836-4a14-aa29-27c2da892398",
      "order_index": 2,
      "title": "Cum crești fără să se rupă și unde ai ajuns",
      "description": null,
      "video_url": "https://www.youtube.com/watch?v=oAJ_ZTcdWpU",
      "pdf_url": null,
      "duration_min": 13,
      "is_published": true
    },
    {
      "id": "6b096509-5128-4cd6-9aad-e3ce2f4ba7e4",
      "module_id": "fceaf485-9834-4e78-8b77-f37a37f798f9",
      "order_index": 0,
      "title": "Documentele firmei tale",
      "description": null,
      "video_url": "https://youtu.be/eqtiz2MSLEA",
      "pdf_url": null,
      "duration_min": 12,
      "is_published": true
    },
    {
      "id": "01779e45-4542-4548-9adf-f27c4d3deb6f",
      "module_id": "fceaf485-9834-4e78-8b77-f37a37f798f9",
      "order_index": 1,
      "title": "Fișa de Post",
      "description": null,
      "video_url": "https://www.youtube.com/watch?v=hhI3-ezBlvo",
      "pdf_url": null,
      "duration_min": 10,
      "is_published": true
    },
    {
      "id": "4667bcea-d9a6-4f50-aca0-e5b4a5fcdf90",
      "module_id": "fceaf485-9834-4e78-8b77-f37a37f798f9",
      "order_index": 2,
      "title": "Instrucțiunea",
      "description": null,
      "video_url": "https://www.youtube.com/watch?v=GtSTmX48Mmc",
      "pdf_url": null,
      "duration_min": 11,
      "is_published": true
    },
    {
      "id": "51741d55-c5eb-4fbf-9c0b-16caab942d27",
      "module_id": "fceaf485-9834-4e78-8b77-f37a37f798f9",
      "order_index": 3,
      "title": "Contoarele",
      "description": "Fără contoare nu conduci — ghicești. Ce măsori, cum măsori și de ce contează.",
      "video_url": "https://youtu.be/OVAm24baaBA",
      "pdf_url": "",
      "duration_min": 18,
      "is_published": true
    },
    {
      "id": "38d1d048-bac5-4178-b800-7203cf7f4337",
      "module_id": "fceaf485-9834-4e78-8b77-f37a37f798f9",
      "order_index": 4,
      "title": "Cum construiești un KPI viu",
      "description": "Un KPI care se folosește, nu unul care stă într-un fișier. Cum îl definești, cum îl urmărești, cum reacționezi.",
      "video_url": "https://youtu.be/itL-bhGLKNE",
      "pdf_url": "",
      "duration_min": 40,
      "is_published": true
    },
    {
      "id": "6bbcd4c0-431d-484a-b2e7-ffb377cab833",
      "module_id": "fceaf485-9834-4e78-8b77-f37a37f798f9",
      "order_index": 5,
      "title": "Tabloul de bord",
      "description": "Cum arată tabloul de bord al unei afaceri conduse cu contoare. 1 pagină, 5 minute luni dimineața.",
      "video_url": "https://www.youtube.com/watch?v=EgQ1OkeBMZA",
      "pdf_url": "",
      "duration_min": 25,
      "is_published": true
    }
  ]
} as const;
